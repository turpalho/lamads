...
chats_handler_router = Router()
chats_handler_router.message.filter(
    F.chat.type.in_(["group", "supergroup", "channel"]))
chats_handler_router.my_chat_member.filter(
    F.chat.type.in_(["group", "supergroup", "channel"]))


@chats_handler_router.message(F.text == "/id")
async def ask_id(message: Message) -> None:
    await message.answer(f"<code>{str(message.chat.id)}</code>")


@chats_handler_router.my_chat_member(
...
async def join_to_chat(event: ChatMemberUpdated,
    ...
    await bot.send_message(
        ...
        text=f"Бот добавлен в чат с ID {event.chat.id}.")
    return